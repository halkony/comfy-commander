...
class ComfyOutput:
    ...
    data: bytes = attrs.field(default=b"")
    ...
    _prompt_id: Optional[str] = attrs.field(default=None, init=False)
    _lazy_loaded: bool = attrs.field(default=False, init=False)
    ...
    def _ensure_data_loaded(self) -> None:
        """Ensure the output data is loaded from the server."""
        if self._lazy_loaded or not self._server or not self._prompt_id:
            return
        
        try:
            # Get the output data from ComfyUI
            output_url = f"{self._server.base_url}/view"
            params = {
            ...
            response = requests.get(output_url, params=params, timeout=self._server.timeout)
            ...
            raise RuntimeError(f"Failed to lazy load data for {self.filename}: {e}")
...
class Workflow:
    ...
    def load_api_json(self, file_path: str) -> None:
        """Load API JSON data from a file and update the workflow.
        
        Args:
            file_path: Path to the API JSON file
        ...
        """
        with open(file_path, 'r', encoding='utf-8') as f:
            self.api_json = json.load(f)
    
    def load_gui_json(self, file_path: str) -> None:
        """Load GUI JSON data from a file and update the workflow.
        
        Args:
            file_path: Path to the GUI JSON file
...
class ComfyUIServer:
                ...
                if isinstance(output_list, list):
                    for output_info in output_list:
                        if isinstance(output_info, dict) and "filename" in output_info:
                            # Create lazy ComfyOutput without downloading data immediately
                            output = ComfyOutput(
                                data=b"",  # Empty data - will be loaded lazily
    ...
    async def _execute_async_internal(self, prompt_id: str, poll_interval: float, timeout: float, workflow: "Workflow") -> ExecutionResult:
        ...
        try:
            ...
            # Get all outputs with workflow metadata (now lazy-loaded)
    ...
    async def execute_concurrent(self, workflows: List["Workflow"], max_concurrent: int = 5, 
                                client_id: str = "comfy-commander", poll_interval: float = 1.0, 
                                timeout: Optional[float] = None, 
                                finished_callback: Optional[callable] = None) -> List[ExecutionResult]:
        """Execute multiple workflows concurrently with a maximum concurrency limit.
        
        Args:
            workflows: List of workflows to execute
            max_concurrent: Maximum number of workflows to execute simultaneously
            client_id: Client identifier for the executions
            poll_interval: How often to check for completion (seconds)
            timeout: Maximum time to wait for each execution (seconds)
            finished_callback: Optional callback function called when each workflow finishes.
                              Called with (workflow_index, result) where workflow_index is the
                              index in the original workflows list and result is the ExecutionResult.
            
        Returns:
            List of ExecutionResult objects in the same order as input workflows
            
        Example:
            def on_finished(result):
                print(f"Workflow finished with status: {result.status}")
                if result.status == "success":
                    result.media[0].save(f"output.png")
            
            results = await server.execute_concurrent(
        ...
        async def execute_single(workflow: "Workflow", index: int) -> ExecutionResult:
            async with semaphore:
                result = await self.execute_async(workflow, client_id, poll_interval, timeout)
                ...
                if finished_callback:
                    try:
                        ...
                        print(f"[WARNING] Finished callback failed for workflow {index}: {e}")
                
                return result
        
        # Execute all workflows concurrently
        tasks = [execute_single(workflow, i) for i, workflow in enumerate(workflows)]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        # Convert exceptions to error ExecutionResults
        final_results = []
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                error_result = ExecutionResult(
                    prompt_id=f"error_{i}",
                    media=MediaCollection(),
                    status="error",
                ...
                # Call callback for error results too
                if finished_callback:
                    try:
                        finished_callback(error_result)
                    except Exception as e:
                        print(f"[WARNING] Finished callback failed for error workflow {i}: {e}")
            else:
